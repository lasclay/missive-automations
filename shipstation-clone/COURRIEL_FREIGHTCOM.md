...
Objet : `API vs ClickShip rate discrepancy + three API questions — Les Produits Lasclay`

---

Hello,

We are Les Produits Lasclay Inc, a Quebec-based manufacturer shipping roughly 8,300 parcels a
year, 80% of them under 500 g. We are moving from a third-party platform to our own in-house
shipping tool, built directly on the Freightcom Customer API (spec 2.10.0). Rating and booking
both work — thank you for the clear specification. Four things came up that we cannot resolve
from our side.
...
shows GST and QST at the expected 14.975%, and the carriers that match include them too. It
is not the residential flag — it is set on both sides. It is not weight or dimensions — those
are identical, and a difference there would not leave two carriers untouched.

**Are negotiated rates applied differently depending on the channel (API vs web)?** If there
is a parameter we should be sending on `POST /rate` to obtain our contracted rates, please
tell us which one.
...
account returns 20 to 22 rates across 153 services searched. We expected the catalogue
endpoint to let us pre-select a service subset (the `services[]` field on `/rate`) in order to
shorten quote times.
...
**Can regular Canada Post services be enabled on our account?** Above 500 g we currently have
no Canada Post option at all, which matters to us: Canada Post is our primary carrier and we
hold our own commercial agreement with them.
...
— to request a delivery confirmation: signature required, proof of age, or Canada Post's
**Do Not Safe Drop**.

This one is important to us. Our current platform applies *Do Not Safe Drop* to every Canada
Post shipment as a standing delivery policy, and we do not want to lose it when we switch.

**Is this configurable at the account level, or is there an accessorial field we have
missed?**
...
We are happy to provide the full JSON request and response for any of the above, or a rate
request ID if that is easier to trace on your side.

Thank you,

Gabriel Gouveia
Les Produits Lasclay Inc — Québec, QC
admin@lasclay.com
