/**
 * Lasclay — revision_ia.js (auto-révision)
 * ---------------------
...
const REVISED_LABEL = process.env.SCAN_LABEL || "019eb935-9b22-7d14-8aeb-614a1e303e24"; // « Draft AI Support »
const ALERTS_ONLY = (process.env.ALERTS_ONLY || "false").toLowerCase() === "true";
const SCAN_LIMIT = parseInt(process.env.SCAN_LIMIT || "0", 10);
...
(async () => {
  ...
  console.log("=== Lasclay revision_ia.js v1 (auto-révision) ===");
  console.log(`Label scanné: ${REVISED_LABEL} | alertes seulement: ${ALERTS_ONLY} | limite: ${SCAN_LIMIT || "aucune"}`);

  const revised = await listByFilter(`shared_label=${REVISED_LABEL}`);
  console.log(`${revised.length} fil(s) portent le label scanné.`);
  if (revised.length === 0) { console.log("Rien à analyser."); return; }

  const items = [];
  let avecAlerte = 0, sansAlerte = 0, errors = 0, scanned = 0;

  for (const conv of revised) {
    if (SCAN_LIMIT > 0 && scanned >= SCAN_LIMIT) { console.log("Limite atteinte."); break; }
    try {
      const [drafts, posts, msgs] = await Promise.all([
        listDrafts(conv.id), listPosts(conv.id), listMessages(conv.id),
      ]);
      // Brouillon IA: le plus ancien créé.
      ...
      // Note ⚠️ IA: c'est un POST posté par « Support IA ».
      const aiNotes = posts
      ...
      // Un fil "avec alerte" = il porte un post ⚠️ NE PAS ENVOYER.
      const aAlerte = aiNotes.some((t) => /NE PAS ENVOYER|VÉRIFIER AVANT|vérifications? requises/i.test(t));
      if (ALERTS_ONLY && !aAlerte) { continue; } // on saute les propres en mode alertes-seulement
      scanned++;
      if (aAlerte) avecAlerte++; else sansAlerte++;
  ...
  // Tri: les fils AVEC alerte d'abord (les plus à risque), puis le reste.
  items.sort((a, b) => (a.aAlerte === b.aAlerte ? 0 : a.aAlerte ? -1 : 1));

  // Rapport
  const lines = [];
  lines.push(`# Révision des brouillons IA — échantillon`);
  lines.push("");
  lines.push(`Généré le ${new Date().toISOString().slice(0, 16).replace("T", " ")}. Auto-révision des brouillons IA (texte + note + fil).`);
  lines.push("");
  lines.push(`**Bilan**: ${items.length} brouillon(s) scanné(s), ${avecAlerte} avec alerte ⚠️, ${sansAlerte} sans alerte.`);
  lines.push("");
  lines.push("---");
  lines.push("");
  for (const it of items) {
    lines.push(`## ${it.client} | ${it.team} | ${it.aAlerte ? "⚠️ alerte" : "○ propre"}`);
  ...
  const filename = `revision_ia_${stamp}.md`;
  await apiPost("/drafts", {
    drafts: {
      ...
      subject: `[NE PAS ENVOYER] Auto-révision IA (${items.length} brouillons, ${avecAlerte} avec alerte)`,
      body: `Rapport d'auto-révision des brouillons IA.<br><br>` +
            `Bilan: ${items.length} brouillon(s), ${avecAlerte} avec alerte, ${sansAlerte} sans alerte.<br><br>` +
  ...
  console.log(`Bilan: ${items.length} brouillon(s), ${avecAlerte} avec alerte, ${sansAlerte} sans alerte, ${errors} erreur(s).`);
