# J7 Media — « Fondation financière » : combien on peut dépenser
...
## Ce que cette source ajoute aux deux autres

| Source | Répond à |
| --- | --- |
| Conférence « Le paid ads en 2026 » | **Comment acheter l'attention** — créatif, structure de compte, hook rate |
| *Sell Like Crazy* (Suby) | **Quoi dire** — l'offre, la garantie, les titres, le courriel |
| **J7 Media, Fondation financière** | **Combien dépenser** — NCAC, cohortes, et le budget comme résultat |

C'est la jambe qui manquait. La conférence donne une règle approximative (« CAC max = marge ×
part cédée ») ; J7 démontre le modèle complet sur un vrai compte client, formules à l'écran.

## La thèse

> « On doit apprendre à compter ! »

> « On devait apprendre à compter pour comprendre exactement ce qui doit se dérouler. […] Ils ne
> connaissent pas exactement tous leurs chiffres, et on ne peut pas [travailler comme ça]. »

Et la posture enseignée face à un client qui annonce « mon objectif est de passer de 1 à 15 M de
chiffre d'affaires cette année » :

> « Bien sûr que non ! On répond plutôt : **Fantastique monsieur le client, laissez-moi vous bâtir
> une fondation financière et je vous reviens rapidement.** »

On ne promet rien avant d'avoir le modèle.
...
## 1. Le chiffrier à monter cette semaine

Douze lignes, une par mois. **Cinq colonnes à saisir**, le reste se calcule :

| Colonne | Où la prendre |
| --- | --- |
| Nouveaux clients | Shopify — clients dont c'est la première commande |
| Dépense Meta (+ Google si applicable) | Gestionnaire de publicités, ou le chiffrier quotidien |
| AOV nouveau client | Shopify, segmenté première commande |
| Marge brute par commande | Fiches COGS + frais de transaction + expédition |
| Revenu des clients existants | Shopify — commandes des clients déjà acquis |

Puis les trois formules :
```
Blended NCAC      = dépense ÷ nouveaux clients
NCAC idéal        = AOV nouveau × marge brute %
Profit par client = (AOV × marge brute %) − NCAC
```

**Aucun nouvel outil à acheter.** Shopify donne les quatre premières colonnes, QuickBooks via le
Finance Proxy donne la cinquième. Lifetimely fait ça automatiquement, mais c'est un abonnement de
plus pour un calcul qu'on peut faire à la main douze fois par an.

## 2. Ce que ça va probablement révéler

Avec une marge brute autour de 73 % (chiffre interne, à revalider avec les fiches COGS courantes) :

| Panier moyen | NCAC idéal (= profit brut de la 1<sup>re</sup> commande) |
| --- | --- |
| 80 $ | **58 $** |
| 120 $ | **88 $** |
| 200 $ | **146 $** |

C'est le plafond au-dessus duquel le profit par client devient négatif. Il est presque certainement
**beaucoup plus haut que ce qu'on ose dépenser aujourd'hui**, et c'est le genre de chiffre qui
...
## 3. La question qui vaut le plus cher : sur quel produit acquérir ?

La démonstration montre trois produits d'entrée du même commerce :

| Produit d'entrée | Réachat | 1<sup>re</sup> commande | À 6 mois |
| --- | --- | --- | --- |
| Box par abonnement | 64 % | 16 € | **57 €** |
| Gelée capillaire | 7 % | 31 € | 36 € |
| Calendrier de l'Avent | 1 % | **93 €** | 94 € |

Le produit au plus gros panier d'entrée est celui qui ne rapporte plus rien après. **Le produit sur
lequel on acquiert décide de la valeur à vie du client** — donc du NCAC qu'on peut se permettre,
produit par produit.

**Chez Lasclay, l'hypothèse à tester dans Shopify sur les deux dernières saisons :**
le client acquis sur les **semences** ou les **mitaines** revient ; celui acquis sur le **manteau**
est une transaction unique à forte marge. Si ça se confirme, ça inverse le réflexe : on peut payer
*plus cher* pour acquérir sur un petit produit que sur un gros.

C'est mesurable sans outil : exporter les commandes, identifier le premier produit acheté par
client, et sommer les commandes suivantes sur 6 et 12 mois.

## 4. Décembre est un mois d'acquisition, pas de fidélisation

Dans le compte montré, la part des commandes de réachat tombe de **57 % en octobre à 35 % en
décembre** — le mois des 7 102 commandes. C'est mécanique : la saison forte fait entrer une masse
de nouveaux clients.

**Conséquence pour Lasclay :** on ne juge pas la rentabilité de novembre-décembre en
novembre-décembre. Le profit de ces clients-là se fait en janvier, au printemps (semences) et à
l'automne suivant. C'est exactement ce que justifient les audiences de rétention à 365 et 730 jours
du guide principal.

Et le taux de réachat d'un client **déjà revenu une fois** : 40,7 % à 30 jours, 69,8 % à 90 jours,
80,1 % à 180 jours, **88,8 % à 365 jours**. Le deuxième achat est le vrai seuil — après, ils
restent. Ce qui fait du flux post-achat (le courriel, section 11 du guide) le levier le plus
rentable de toute la saison.

## 5. Ce que J7 corrige dans le plan de saison

| Le plan disait | J7 ajoute |
| --- | --- |
| CAC max = marge × part qu'on accepte de céder | Le vrai plafond est le **profit brut de la première commande**, et il se vérifie mois par mois |
| Regarder le ROAS avec prudence | Le remplacer par le **POAS** — profit sur la dépense |
| Fixer un budget de saison | Ne pas le fixer : objectif de nouveaux clients × NCAC = budget |
| Créer des audiences longues (365-730 j) | La raison chiffrée : 59 % du revenu vient des clients déjà acquis |
| (rien sur le choix du produit à pousser) | La cohorte par produit décide du NCAC acceptable |

---

## Source

`Fondation Financière - J7 Meida.mp4` (dossier Drive, 1,36 Go, 39:46, propriété de
adalmas@j7media.com). Diapositives extraites en 160 trames haute résolution, transcription locale
(faster-whisper). Les chiffres du compte client présenté appartiennent à un tiers : référence de
méthode à usage interne, à ne pas rediffuser.

**Version visuelle** (section 02 du guide) :
