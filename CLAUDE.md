...
- `merge.js` : détection des fils en doublon (label « À fusionner » ; fusion irréversible
  derrière `MERGE=true`, et **sur des fils ouverts seulement**). Même adresse = même client,
  sans condition de date ; les garde-fous v2.1 visent les liens entre adresses différentes,
  qui soudaient deux clientes. Tests : `node merge_test.js`.
