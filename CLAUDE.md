...
| `proxygen` | General Proxy : ShipStation, Omnisend, Klaviyo, Shopify (cartes-cadeaux) |
...
- **Shopify Admin** (`SHOPIFY_STORE` + `SHOPIFY_CLIENT_ID`/`SHOPIFY_CLIENT_SECRET` côté Render) :
  cartes-cadeaux (lecture, émission ⚠️ argent réel, envoi, désactivation) et clients. C'est LE
  chemin pour émettre une carte-cadeau : le connecteur Shopify MCP refuse toute écriture sur les
  cartes-cadeaux par politique. Portées à publier dans le Dev Dashboard : `write_gift_cards`,
  `write_customers`. `node connectors_client.js shopify diag` dit ce qui manque.
