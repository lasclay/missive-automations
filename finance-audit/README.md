...
| `design_pdf.py` | Le système visuel partagé des trois documents PDF : palette, formatage des nombres à la française, graphiques SVG posés à la main, feuille de style, et `rendre()` qui assemble, imprime par Chromium et recompresse. Une palette recopiée diverge à la première retouche. |
| `memo_wassim.py` | Le mémo de Wassim : le même modèle lu sur le scénario optimiste, les trois lectures côte à côte, et ce que l'Europe ajouterait. Ni couverture du service de la dette ni covenants : ce n'est pas son sujet. |
| `proposition_wassim.py` | La proposition de partenariat, version d'août 2026 : part de 30 % sur Lasclay au complet, apport de 100 000 $ CAD, plafond fixe de 500 000 $, option de renouvellement à 700 000 $, et les petits caractères qui disent que c'est du capital de risque. |
...
| `fix_bc.py` | Un **troisième scénario** sur `Inputs!C70` : 1 conservateur, 2 ambitieux, 3 optimiste. Le troisième remet le classeur sur la trajectoire de la proposition de juillet 2026 (4,17 M$ en 2028-2029), mais portée par les deux moteurs — le canal détail se déploie au rythme ambitieux dans les trois lectures. L'écart appliqué aux facteurs de croissance est résolu par bissection. |
| `fix_bd.py` | La **RS&DE** remise sur le relevé de dépenses de 2026. Le classeur portait 73 865 $, soit 55 % du salaire de deux personnes ; la base admissible réelle est de 246 193 $ sur trois projets, pour un cas central retenu de 140 000 $. Le raccord se vérifie au dollar — relevé et rangée 45 donnent le même solde de salaires de R&D au grand livre, 109 014 $. Les exercices projetés appliquent la formule du relevé aux dépenses de R&D que le modèle budgète, plutôt que le ratio de 20 % des charges totales, qui a été observé pendant que la production était au Québec. L'encaissement passe d'un versement unique de mai à deux virements, 35 % en avril et 65 % en juin. |
| `fix_be.py` | L'avance de l'actionnaire portée de 80 000 $ à 90 000 $ (`Inputs!C116`). Les 10 000 $ tombent sur le seul mois tendu des trente-sept projetés, août 2026 : le tirage de la marge passe de 131 134 $ à 121 200 $ sur une facilité de 150 000 $. En contrepartie ils se remboursent en 2026-2027, et la couverture du service de la dette de cet exercice recule de 1,02 à 0,98. |
| `fix_bf.py` | **Le budget de caisse complété.** La vue directe s'écartait de la chaîne de trésorerie du résultat de -105 340 $ à +67 136 $ sur 36 des 36 mois projetés. Le résidu se décomposait en quatre morceaux : impôts versés, cinq postes de fonds de roulement (fournisseurs, frais courus, cartes de crédit, taxes de vente, frais payés d'avance) et l'ajustement de calendrier BMB, présent à la caisse et absent du bilan. Rangées 27 et 29 ajoutées, rangée 25 retirée des mois projetés : le contrôle tombe à zéro sur 36/36. Les colonnes réelles ne sont pas touchées, le bilan d'août 2026 lisant « Account payable » qui lit lui-même ce budget. |
| `archive_wassim.py` | Reconstitue, nomme et documente la version optimiste de juillet 2026 — celle d'où viennent les chiffres de ventes de la proposition à Wassim. Pose en tête une feuille « AVERTISSEMENT » qui dit d'où sortent ces chiffres, ce qui ne concorde pas, et ce que le classeur porte encore de non corrigé. |
